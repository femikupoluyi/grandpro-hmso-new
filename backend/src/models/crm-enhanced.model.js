/**
 * Enhanced CRM Database Models
 * Complete Owner and Patient CRM functionality
 */

const { DataTypes } = require('sequelize');
const sequelize = require('../config/sequelize');

// Owner CRM Model - Track hospital owners
const Owner = sequelize.define('Owner', {
  ...
  tableName: 'crm_owners',
  timestamps: true
});

// Patient CRM Model - Track patients across hospitals
...
const CommunicationLog = sequelize.define('CommunicationLog', {
  ...
  tableName: 'crm_communication_logs',
  timestamps: true
});
...
const Campaign = sequelize.define('Campaign', {
  ...
  segment_criteria: {
    type: DataTypes.JSON,
    comment: 'Criteria for targeting specific segments'
  },
  channels: {
    type: DataTypes.JSON,
...
const LoyaltyTransaction = sequelize.define('LoyaltyTransaction', {
  ...
  reference_type: {
    type: DataTypes.STRING(50),
    comment: 'e.g., appointment, payment, referral'
  },
  reference_id: {
    type: DataTypes.INTEGER,
  ...
  tableName: 'crm_loyalty_transactions',
  timestamps: true
});

// Payout Model for Owners
const Payout = sequelize.define('Payout', {
  ...
  tableName: 'crm_payouts',
  timestamps: true
});

// Define associations
Owner.hasMany(Payout, { foreignKey: 'owner_id' });
Payout.belongsTo(Owner, { foreignKey: 'owner_id' });

Patient.hasMany(Appointment, { foreignKey: 'patient_id' });
Appointment.belongsTo(Patient, { foreignKey: 'patient_id' });

Patient.hasMany(LoyaltyTransaction, { foreignKey: 'patient_id' });
LoyaltyTransaction.belongsTo(Patient, { foreignKey: 'patient_id' });
